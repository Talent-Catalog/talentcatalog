select
    distinct candidate0_.id as id1_1_0_,
             ...
             (select count(cd.id)
              from candidate c
                       inner join candidate_dependant cd on c.id = cd.candidate_id
              where c.id = candidate0_.id
              group by c.id) as formula1_0_,
...
from candidate candidate0_
         inner join users user1_ on candidate0_.user_id = user1_.id
         inner join partner partnerimp2_ on user1_.partner_id = partnerimp2_.id
         inner join country country3_ on candidate0_.nationality_id = country3_.id
         inner join country country4_ on candidate0_.country_id = country4_.id
         inner join education_level educationl5_ on candidate0_.max_education_level_id = educationl5_.id
where candidate0_.status in ('active','unreachable','incomplete','pending')
  and user1_.partner_id in (1,3,4,5,6,8,16)
order by candidate0_.id desc nulls last
limit ? offset ?;