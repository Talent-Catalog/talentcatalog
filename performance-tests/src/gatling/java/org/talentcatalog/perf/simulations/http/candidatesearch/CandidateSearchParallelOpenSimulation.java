...
/**
 * Runs a parallel A/B comparison of candidate search endpoints using Gatling's <b>open workload model</b>.
 *
 * <h2>Goal</h2>
 * Generates a constant arrival rate (requests started per second) split approximately 50/50 between:
 * <ul>
 *   <li><b>A (NEW)</b>: {@link NewEndpointLoopScenario}</li>
 *   <li><b>B (OLD-FETCH)</b>: {@link OldEndpointLoopScenario}</li>
 * </ul>
 *
 * <p>Each virtual user logs in once (inside the scenario) and then loops {@code searchRepeats} search calls
 * with random think time, so the test isolates search endpoint performance rather than login throughput
 * or rate limits.</p>
 *
 * <h2>Phases</h2>
 * <ol>
 ...
 * }</pre>
 */
public class CandidateSearchParallelOpenSimulation extends CandidateSearchBaseSimulation {
   ...
   */
  public CandidateSearchParallelOpenSimulation() {
    if (TOTAL_USERS_PER_SEC <= 0.0) throw new IllegalArgumentException("totalUsersPerSec must be > 0");
    if (RAMP_SECONDS < 0) throw new IllegalArgumentException("rampSeconds must be >= 0");
    if (WARMUP_SECONDS < 0) throw new IllegalArgumentException("warmupSeconds must be >= 0");
    if (MEASURE_SECONDS <= 0) throw new IllegalArgumentException("measureSeconds must be > 0");

    if (SEARCH_REPEATS <= 0) throw new IllegalArgumentException("searchRepeats must be > 0");
    if (SEARCH_MIN_PAUSE_SECONDS < 0) throw new IllegalArgumentException("searchMinPauseSeconds must be >= 0");
