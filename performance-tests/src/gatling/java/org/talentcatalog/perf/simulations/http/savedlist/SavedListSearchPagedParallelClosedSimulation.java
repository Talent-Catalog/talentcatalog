...
/**
 * Runs a parallel A/B comparison of saved-list candidate search (paged) implementations using
 * Gatling's
 * <b>closed workload model</b>.
 *
 * <h2>Goal</h2>
 * Keep a constant number of concurrent users active across both implementations for the full test
 * duration, split approximately 50/50 between:
 * <ul>
 *   <li><b>A (NEW)</b>: {@link NewSearchPagedLoopScenario}</li>
 *   <li><b>B (OLD-FETCH)</b>: {@link OldSearchPagedLoopScenario}</li>
 * </ul>
 *
 * <p>Each virtual user logs in once (via the scenario chain) and then loops {@code searchRepeats}
 * paged search calls with random think time between iterations.</p>
 *
 * <h2>Duration</h2>
 * <p>Total runtime is {@code warmupSeconds + measureSeconds}. This simulation also sets
 * {@code maxDuration(Duration)} with a small buffer to ensure Gatling shuts down cleanly and writes reports.</p>
 *
 ...
 *   -PsimClass=org.talentcatalog.perf.simulations.http.savedlist.SavedListSearchPagedParallelClosedSimulation \
 ...
 * }</pre>
 */
public class SavedListSearchPagedParallelClosedSimulation extends
   ...
   */
  public SavedListSearchPagedParallelClosedSimulation() {
    ...
    var scnA = NewSearchPagedLoopScenario.build(
        listId, payloadPath, "[NEW]", SEARCH_REPEATS, SEARCH_MIN_PAUSE_SECONDS,
        SEARCH_MAX_PAUSE_SECONDS
    );

    var scnB = OldSearchPagedLoopScenario.build(
        listId, payloadPath, "[OLD]", SEARCH_REPEATS, SEARCH_MIN_PAUSE_SECONDS,
        SEARCH_MAX_PAUSE_SECONDS
    );
