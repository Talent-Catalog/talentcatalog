...
export class AssignTasksCandidateComponent implements OnInit {
  ...
  getAllTasks() {
    ...
    this.taskService.listTasks().subscribe(
      (tasks: Task[]) => {
        // TODO: Refactor this filtering logic once we have a proper feature flag or metadata
        // to identify and hide auto-created tasks like "duolingoTest" and "claimCouponButton".
        // Ideally, the backend should provide a flag (e.g., `isAutoGenerated`) for tasks
        // that can be hidden based on user roles or settings.
        // Currently always filter out "duolingoTest" based on task name
        tasks = tasks.filter(task => task.name !== "duolingoTest");
        ...
        if (!this.authorizationService.isDefaultSourcePartner()) {
          ...
          tasks = tasks.filter(task => task.name !== "claimCouponButton");
        }
  ...
  searchTypeOrName = (searchTerm: string, item: any) => {
    return item.taskType.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1 ||
      item.displayName && (item.displayName.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1);
  }
