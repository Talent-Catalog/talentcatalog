...
/**
 * Filters out a candidate's properties in a DtoBuilder based on a user's role and associated
 * partner.
 * <p/>
 * Broadly, a user can only see public properties of candidates who do not belong to the user's
 * partner. The one exception is users with role systemadmin - they can always see everything. If a
 * candidate does belong to the user's partner, then the properties can be viewed depend on the role
 * of that user.
 *
 * @author John Cameron
 */
public class PartnerAndRoleBasedDtoPropertyFilter implements DtoPropertyFilter {
  ...
  @Nullable
  private Set<Candidate> fullyVisibleCandidates;
  @Nullable
  private Set<User> fullyVisibleUsers; //Computed from fullyVisibleCandidates
  @Nullable
   ...
   *
   * @param partner                    Partner associated with the user requesting the candidate
   *                                   data.
   ...
   * @param publicProperties           Names of properties that are always returned (ie never
   *                                   filtered out)
