 ...
 *
 * <p>See https://docs.spring.io/spring-security/site/docs/3.2.0.RC2/reference/htmlsingle/#jc also
 * https://www.marcobehler.com/guides/spring-security
 *
 * <p>Summary of TBB Talent Catalog security:
 *
 * <ul>
 *   <li>We manage our own users and passwords, stored in the users table of the database
 *   <li>At login we issue JSON Web Tokens (JWTs) which appear on each HTTP request in the
 *       Authorization header as a Bearer token. See
 *       https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Authorization and
 *       https://developer.mozilla.org/en-US/docs/Web/HTTP/Authentication#authentication_schemes.
 *       This is OAUTH 2.0 with us acting as the Authorization server - see
 ...
 *
 * <ul>
 *   <li>{@link JwtTokenProvider} generates JWT tokens
 *   <li>{@link JwtAuthenticationFilter} is a filter which checks incoming JWT tokens, validating
 *       them.
 *   <li>{@link TcUserDetailsService} implements UserDetailsService providing access to the user
 *       table in our database
 *   <li>{@link TcAuthenticationProvider} implements AuthenticationProvider passing in our wired in
 *       instance of the above TcUserDetailsService, and the PasswordEncoder defined below in {@link
 *       #passwordEncoder()}.
 * </ul>
 */
@Configuration
...
public class SecurityConfiguration {
  ...
  @Autowired private TcUserDetailsService userDetailsService;
  ...
  public SecurityFilterChain filterChain(
    ...
    http
        ...
        .and()
        .authorizeRequests()
        .requestMatchers("/backend/jobseeker")
        .permitAll()
        .requestMatchers("/api/portal/auth")
        .permitAll()
        .requestMatchers("/api/portal/auth/**")
        .permitAll()
        .requestMatchers("/api/portal/branding")
        .permitAll()
        .requestMatchers("/api/portal/user/reset-password-email")
        .permitAll()
        .requestMatchers("/api/portal/user/check-token")
        .permitAll()
        .requestMatchers("/api/portal/user/reset-password")
        .permitAll()
        .requestMatchers("/api/portal/language/system/**")
        .permitAll()
        .requestMatchers("/api/portal/language/translations/**")
        .permitAll()
        ...
        .requestMatchers("/api/admin/auth/**")
        .permitAll()
        .requestMatchers("/api/admin/branding")
        .permitAll()
        .requestMatchers("/api/admin/user/reset-password-email")
        .permitAll()
        .requestMatchers("/api/admin/user/check-token")
        .permitAll()
        .requestMatchers("/api/admin/user/reset-password")
        .permitAll()
        .requestMatchers("/")
        .permitAll()
        .requestMatchers("/published/**")
        .permitAll()
        ...
        // SEE CANDIDATE FILE ATTACHMENTS. ADMIN/SOURCE PARTNER ADMIN ALLOWED. READ ONLY has access
        // BUT has the data restricted in the DTO based on role.
        ...
        .hasAnyRole(
            "SYSTEMADMIN", "ADMIN", "PARTNERADMIN", "SEMILIMITED", "LIMITED", "USER", "READONLY")
    ...
    //        .requiresChannel().requestMatchers( r -> r.getHeader("X-Forwarded-Proto") !=
    // null).requiresSecure()
    ...
    // See
    // https://docs.spring.io/spring-security/site/docs/5.2.0.RELEASE/reference/html/default-security-headers-2.html#webflux-headers-csp
