...
class AuthAdminApiTest extends ApiTestBase {
    ...
    @Test
    @DisplayName("login fails with unauthorized when client id is invalid")
    void loginFailsClientIdCheck() throws Exception {
        given(userService.login(any(AuthProfile.class))).willReturn(user);

        Exception loginException = new UsernameNotFoundException("Unrecognized client id");
        willThrow(loginException)
            .given(oAuth2UserService)
            .checkUserClientId(eq(user), eq(OAuth2UserService.OAUTH_TC_ADMIN_CLIENT_ID));

        doLoginAndVerifyFails(USER_NOT_FOUND_CODE, loginException.getMessage());

        verify(userService, never()).createAuthenticationResponse(any(User.class));
    }
    ...
    void loginFailsUserServiceException(Throwable loginException, String errorCode) throws Exception {
        given(userService.login(any(AuthProfile.class))).willThrow(loginException);
