...
public class CandidateVisaCheckAdminApiTest extends ApiTestBase {
  ...
  @Autowired
  CandidateVisaCheckAdminApi candidateVisaCheckAdminApi;
  ...
  void getByIdSucceeds() throws Exception {
    ...
    mockMvc.perform(get(BASE_PATH + "/" + CANDIDATE_ID)
        ...
        .andExpect(
            jsonPath("$.englishThresholdNotes", is("These are some english threshold notes.")))
        .andExpect(jsonPath("$.healthAssessment", is("Yes")))
        .andExpect(
            jsonPath("$.healthAssessmentNotes", is("These are some health assessment notes.")))
        .andExpect(jsonPath("$.characterAssessment", is("No")))
        .andExpect(jsonPath("$.characterAssessmentNotes",
            is("These are some character assessment notes.")))
        ...
        .andExpect(
            jsonPath("$.pathwayAssessmentNotes", is("These are some pathway assessment notes.")));
  ...
  void listByCandidateIdSucceeds() throws Exception {
    ...
    mockMvc.perform(get(BASE_PATH + LIST_PATH.replace("{id}", Long.toString(CANDIDATE_ID)))
        ...
        .andExpect(
            jsonPath("$.[0].englishThresholdNotes", is("These are some english threshold notes.")))
        .andExpect(jsonPath("$.[0].healthAssessment", is("Yes")))
        .andExpect(
            jsonPath("$.[0].healthAssessmentNotes", is("These are some health assessment notes.")))
        .andExpect(jsonPath("$.[0].characterAssessment", is("No")))
        .andExpect(jsonPath("$.[0].characterAssessmentNotes",
            is("These are some character assessment notes.")))
        ...
        .andExpect(jsonPath("$.[0].pathwayAssessmentNotes",
            is("These are some pathway assessment notes.")));
  ...
  void createVisaCheckSucceeds() throws Exception {
    ...
    verify(candidateVisaService).createVisaCheck(anyLong(),
        any(CreateCandidateVisaCheckRequest.class));
  }
